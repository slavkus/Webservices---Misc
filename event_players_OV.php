...
&& isset($_GET['approved']) && !empty($_GET['approved']))
{
	...
	$user_by_event->user_id = $_GET['user_id'];
	$user_by_event->event_id = $_GET['event_id'];
	$user_by_event->approved = $_GET['approved'];

	$stmtAll = $user_by_event->getAllUserByEvent();
	$stmtApp = $user_by_event->getApprovedUserByEvent();
	$bundle_user_event = array();
		
		while ($rowApp = $stmtApp->fetch(PDO::FETCH_ASSOC))
        {
			extract($rowApp);
			$bundle_item_App = array (
				"user_id" => $user_id,
				"event_id" => $event_id,
            ...
            array_push($bundle_user_event, $bundle_item_App);
		}
